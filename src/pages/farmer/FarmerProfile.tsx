...
const FarmerProfile = () => {
  ...
  const [profile, setProfile] = useState({
    ...
    bio: 'Experienced farmer with over 15 years of farming. Specialized in organic farming methods.',
  });

  const [kycDocuments, setKycDocuments] = useState({
    aadhaar: { uploaded: true, verified: true, fileName: 'aadhaar_front.jpg' },
    kisan: { uploaded: true, verified: false, fileName: 'kisan_id.pdf' },
  ...
  const handleSave = () => {
    ...
    toast({
      ...
      description: currentLanguage === 'en' ? 'Your changes have been saved.' : 'आपके बदलाव सहेजे गए।',
    });
  };
  ...
  const handleKycUpload = (docType: string) => {
    setKycDocuments(prev => ({
      ...prev,
      [docType]: { uploaded: true, verified: false, fileName: `${docType}_doc.pdf` }
    }));
    toast({
      title: currentLanguage === 'en' ? 'Document Uploaded' : 'दस्तावेज़ अपलोड',
      description: currentLanguage === 'en' ? 'Your document is under review.' : 'आपका दस्तावेज़ समीक्षाधीन है।',
    });
  };
          ...
          <TabsContent value="kyc" className="space-y-6">
            ...
            <div className="card-elevated p-6">
              <div className="flex items-center gap-3 mb-6">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${farmer.kycStatus === 'approved' ? 'bg-success/10' : 'bg-warning/10'}`}>
